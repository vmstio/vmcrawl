...
def load_from_database(user_choice):
    ...
    query_map = {
        ...
        "5": "SELECT rd.domain FROM raw_domains rd WHERE rd.nodeinfo = 'mastodon' AND NOT EXISTS (SELECT 1 FROM mastodon_domains md WHERE md.domain = rd.domain) AND (rd.nxdomain IS NOT NULL) AND rd.reason <> 'API' ORDER BY rd.domain;",
